...
func Root(ctx context.Context, service system.Service) (err error) {
	...
	container.AddSingleton("stream", func(c di.Container) (any, error) {
		return jetstream.NewStream(service.Config().Nats.Stream, service.JS(), c.Get("logger").(*slog.Logger)), nil
	})
	...
	container.AddScoped("domainEventHandlers", func(c di.Container) (any, error) {
		return logging.LogEventHandlerAccess[ddd.AggregateEvent](
			...
			"DomainEvents", c.Get("logger").(*slog.Logger),
		), nil
	})
	...
	container.AddScoped("commandHandlers", func(c di.Container) (any, error) {
		return logging.LogCommandHandlerAccess[ddd.Command](
			...
			"Commands", c.Get("logger").(*slog.Logger),
		), nil
	})
