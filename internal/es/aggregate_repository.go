...
func (r aggregateRepository[T]) Load(ctx context.Context, aggregateID string) (T, error) {
	var agg T
	v, err := r.registry.Build(r.aggregateName, ddd.SetID(aggregateID), ddd.SetName(r.aggregateName))
	if err != nil {
		return agg, err
	}
