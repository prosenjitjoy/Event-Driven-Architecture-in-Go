...
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "malldb" <<-EOSQL
  ...
  CREATE INDEX ordreing_unpublished_idx ON ordering.outbox(published_at) WHERE published_at IS NULL;

  GRANT USAGE ON SCHEMA ordering TO malldb_user;

  GRANT INSERT, UPDATE, DELETE, SELECT ON ALL TABLES IN SCHEMA ordering TO malldb_user;
EOSQL